...
_AC_CATEGORY_RE = re.compile(
    ...
    r'электрические тепловые пушки|газовые тепловые пушки|тепловентилятор|'
    r'маслонаполненн\w*\s+радиатор|'
    r'воздушные и тепловые завес|инфракрасные обогревател|'
    r'осушители воздуха|увлажнители воздуха|'
    r'компактные моноблочные приточн|бытовая приточная вентиляц|'
    r'приточные очистители воздуха|'
    # Отопление здания (2026-08-29, по просьбе владельца — берём и «под
    # заказ»): радиаторы и тёплый пол. Аксессуары к ним отсекает
    # _AC_EXCLUDE_RE (комплектующие, подложки, кронштейны, клапаны).
    r'^радиаторы (секционные|стальные|биметалл|алюмин|отопления|чугунные)|'
    r'нагревательные маты для теплого пола|'
    r'нагревательный кабель для теплого пола|'
    r'терморегуляторы для теплого пола',
    re.IGNORECASE,
)
...
# Название категории Rusklimat → наша мастер-категория. Значение — либо breez_id
# (категория пришла из дерева Бриза), либо title нашей собственной категории,
# которой у Бриза нет (радиаторы, тёплый пол). Порядок важен: берётся первое
...
_RK_CATEGORY_MAP = (
    ...
    ('маслонаполненн',          34),   # так они называются у Rusklimat
    ('тепловые пушки',          37),
    ('тепловентилятор',         31),
...
def _master_category_for(name):
    ...
    """
    n = (name or '').lower()
    for needle, target in _RK_CATEGORY_MAP:
        if needle not in n:
            continue
        if isinstance(target, int):
            return Category.objects.filter(breez_id=target).first()
        return Category.objects.filter(title=target).first()
