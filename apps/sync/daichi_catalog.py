...
# Если нашей категории с таким title нет или она выключена — товар пропускается
# (sync_enabled управляется в admin и командой enable_retail_categories).
# Расширено 2026-08-29: до этого синкались только бытовые сплиты, а в Крыму у
# Daichi лежат ещё полупром (58 позиций), котлы (31) и мультисистемы.
_GOODGROUP_TO_TITLE = {
    ...
    'Мультисистемы': 'Мульти сплит-системы',
    'Котлы': 'Отопительные котлы',
    'Увлажнители': 'Бытовые увлажнители воздуха',
    'Осушители': 'Бытовые осушители воздуха',
    'Компактная вентиляция': 'Компактные моноблочные вентиляционные установки',
    # Добавлено 2026-08-30: розница, которой не было в дереве.
    'Бытовые вентиляторы': 'Бытовые вентиляторы',
    'Воздухоочистители': 'Воздухоочистители',
    'Оконные кондиционеры': 'Оконные кондиционеры',
}

# Группа «Обогреватели» у Daichi смешанная — конвекторы, масляные радиаторы,
# ИК-панели и тепловентиляторы вперемешку. Раскладываем по ATTR_RUS_NAME_AX:
# поставщик держит там человеческое название типа товара.
_RUS_NAME_TO_TITLE = {
    'конвектор электрический':    'Конвекторы',
    'радиатор масляный':          'Масляные радиаторы',
    'обогреватель инфракрасный':  'Инфракрасные обогреватели',
    'тепловентилятор керамический': 'Тепловентиляторы',
}

# Группы, товары которых раскладываются по _RUS_NAME_TO_TITLE, а не целиком
# в один раздел.
_SPLIT_BY_RUS_NAME = {'Обогреватели'}

# Требование «только готовый комплект» осмысленно для сплит-систем: иначе в
# каталог попадут отдельные внутренние и наружные блоки, которые розничному
# покупателю не нужны. У котла или увлажнителя тип товара другой — там фильтр
# не применяем, иначе группа синкнется пустой.
_KIT_REQUIRED_GROUPS = {
    'Бытовые сплит-системы', 'Полупромышленные сплит-системы', 'Мультисистемы',
}
...
def _resolve_category(params):
    ...
    if goodgroup in _SPLIT_BY_RUS_NAME:
        rus_name = (params.get('ATTR_RUS_NAME_AX') or '').strip().lower()
        target_title = _RUS_NAME_TO_TITLE.get(rus_name)
    else:
...
def _is_kit(params):
    ...
    Для сплит-систем — только готовый комплект; для остальных групп (котлы,
    увлажнители, осушители, вентиляция) тип товара не проверяем.
    """
    goodgroup = (params.get('ATTR_L_GOODGROUP') or '').strip()
    if goodgroup not in _KIT_REQUIRED_GROUPS:
        return True
