...
и в разных форматах. Поэтому is_heat_pump считается по этому диапазону (плюс прямое
упоминание теплонасоса в названии), а характеристика поставщика лишь дополняет его —
см. _is_air_to_air_heat_pump.
...
HEATING_SPEC_TITLES = (
    ...
    'Нижний диапазон рабочих температурных границ, нагрев, °C',    # Профконд (JAC)
)
...
# Поставщик называет тепловой насос прямо в названии товара. Такие позиции
# приходят БЕЗ характеристики «Тепловой насос» и без диапазона обогрева — по
# порогу их не поймать (живой пример с прода 2026-09-12: три «Инверторный
# тепловой насос настенного типа серии AKEBONO NORDIC (R32)» лежали в бытовых
# сплит-системах и в подборку не попадали).
_TITLE_HEAT_PUMP_RE = re.compile(r'теплов\w*\s+насос', re.IGNORECASE)

# ...но бассейновый теплонасос — воздух-ВОДА: он греет воду, а подборка
# собирает воздух-воздух. Такие в неё не идут, даже когда поставщик объявил
# их тепловыми насосами.
_TITLE_NOT_AIR_TO_AIR_RE = re.compile(r'для\s+бассейн|воздух\s*[-—−/]\s*вода', re.IGNORECASE)
...
def _named_heat_pump(title):
    """True, если сам НАЗВАНИЕ товара объявляет его теплонасосом воздух-воздух."""
    text = title or ''
    if not _TITLE_HEAT_PUMP_RE.search(text):
        return False
    return not _TITLE_NOT_AIR_TO_AIR_RE.search(text)


def _is_air_to_air_heat_pump(product, temp):
    """True, если товар — тепловой насос воздух-воздух (признак подборки).
    ...
    (проверено на проде 2026-09-12).

    Считается по двум признакам — диапазон обогрева ниже HEAT_PUMP_THRESHOLD
    (тот же порог, по которому отбирается подборка, см. collections.py) либо
    прямое объявление в названии. И только для сплит-систем: тепловой насос
    воздух-воздух — это ярлык на сплите, а не на конвекторе или котле, у
    которых тоже бывает отрицательная рабочая температура.
    """
    if product.kind != Product.KIND_SPLIT_SYSTEM:
        return False
    if temp is not None and temp <= HEAT_PUMP_THRESHOLD:
        return True
    return _named_heat_pump(product.title)
...
def apply_heating_fields(product, declared=False):
    ...
    is_pump = (
        ...
        or _is_air_to_air_heat_pump(product, temp)
    )
