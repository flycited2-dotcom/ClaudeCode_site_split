...
# ищутся текстом.
#
# «Тепловой насос» исключён по той же причине, но вреда от него было больше:
# характеристика приходит только от Бриза и заполнена у 758 товаров из 14413,
# значение «да» — у 5. На странице подборки «Тепловые насосы» панель показывала
# «нет — 36, да — 1», т.е. фильтр отсекал 58 её товаров из 59 (прод,
# 2026-09-12). Тот же смысл честно отдают захардкоженная фасета «Работает на
# обогрев до −20/−25/−30» и поле Product.is_heat_pump (см. heating.py).
_EXCLUDED_TITLES = {'бренд', 'тепловой насос'}
